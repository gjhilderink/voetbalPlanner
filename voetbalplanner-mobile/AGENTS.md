...
5. Author the patch in `dsl/edit.dart` using `findByPath(...)`:
   ...
   ```
6. Run `flutterflow ai test`, then `flutterflow ai validate`, then `flutterflow ai run`. The `flutterflow ai test` wrapper runs `dart test` and additionally records compile/test outcomes for the FlutterFlow AI dashboard. Plain `dart test` still works but won't be tracked.
...
### Theme first
Always set up a theme before building UI. Define the color scheme with `app.themeColor()`, the type scale with `app.typography(...)`, and design tokens with `app.spacingToken/radiusToken/shadowToken(...)`. When building new UI from scratch, prefer theme tokens (`Colors.primary`, `Colors.secondary`, `Colors.secondaryText`, etc.) so the design stays cohesive. The full theme/design-system surface (palette, typography, tokens, custom fonts/icons, scrollbar, pull-to-refresh) is in `references/theme_design_system_dsl.dart`. For a smaller "minimum viable theme + form layout" pattern, see `references/styled_profile_dsl.dart`.

**Color edits: respect the scope of the request.** Theme slots cascade — changing `Colors.primary` changes every widget bound to it (AppBar, buttons, text colors in other components, …). When a user asks to change a *specific* widget (or a small set of widgets) to a *specific* color (e.g. "make this button purple"), set that color directly on the node with `Colors.hex(0xFFA020F0)` — do NOT mutate `app.themeColor(...)`. Reserve `app.themeColor(...)` for the cases where the user explicitly asks for a brand / app-wide / theme change ("change our primary color to purple", "switch the brand color", "redo the theme"). When unsure between the two, use a node-level hex: it's reversible and local. A theme-slot edit is a project-wide mutation and must be a deliberate, requested change.
...
### Page metadata edits (route, description, auth)
In an edit script you cannot re-run `app.page(...)` on a bound project. To change the route on an existing page use the brownfield helpers — they normalize the path (strip leading/trailing slashes) and reject reserved Dart keywords (`/new`, `/null`, `/return`, etc.) that the FlutterFlow editor would otherwise flag:

```dart
app.raw((project) {
  setPageRoute(project, pageName: 'NewMeetingPage', route: '/new-meeting');
  setPageRequiresAuth(project, pageName: 'NewMeetingPage', requiresAuth: true);
  updatePage(project, name: 'NewMeetingPage', description: 'Pick a contact to record');
});
```

Do NOT write `page.ensurePageRouteSettings().routePath = '/new-meeting'` directly — that bypasses normalization, leaves the leading slash in the stored value, and the editor will render the route as `//new-meeting`.
